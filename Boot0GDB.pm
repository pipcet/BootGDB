...
sub guess_macro_type
{
  ...
  for my $internal (keys %{$internals}) {
	...
	if ($#potential_types == 0 and $return_type) {
	  my $ctype = $type->c_emit;
	  my $rctype = $return_type->c_emit;

	  if (defined $ctype and defined $rctype) {
	    $self->{c_src} .=  "$rctype macro_$macro($ctype arg) { return " . $self->execute_command_to_string("py print macro_string('$macro', '$linespec')") . "; }\n";
	  }
	}
      }
    }
...
#  my $e = Boot0GDB::Expression->new($self, '(int)(((int *)$x)+1)');
  ...
  print "/* DO NOT EDIT. AUTOGENERATED CODE FOLLOWS */\n" . $self->{c_src};
